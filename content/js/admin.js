...
/*!
 * Custom .JS
 * For maintaining the toggle state.
 */
jQuery(document).ready(function() {
  var custom_uploader;

  jQuery('#Signals_Upload_Btn').click(function(e) {
    e.preventDefault();
    ...
    custom_uploader = wp.media.frames.file_frame = wp.media({
        title: 'Choose Background Cover',
        button: {
            text: 'Choose Background Cover'
        },
        multiple: false
    });
    ...
    custom_uploader.on('select', function() {
        ...
        jQuery('#signals_CSMM_bg').val(attachment.url);
    });
  ...
  var $state = jQuery.cookie('Signals_CSMM_Menu');

  if ($state) {
    jQuery('.Signals_Main_Menu li a').removeClass("active");
    jQuery('a[href="' + $state + '"]').addClass("active");

    jQuery($state).fadeIn();
  } else {
    jQuery('.Signals_Main_Menu li:first a').addClass("active");
    jQuery('.Signals_Tile:first').fadeIn();
  }

  jQuery('.Signals_Main_Menu li a').click(function(e) {
    e.preventDefault();

    jQuery.removeCookie('Signals_CSMM_Menu', { path: '/' });
    ...
    jQuery('.Signals_Tile').hide();
    jQuery($tab).fadeIn();

    jQuery.cookie('Signals_CSMM_Menu', $tab, { path: '/' });
  });

  jQuery('.Signals_Mobile_Menu a').click(function() {
    jQuery('.Signals_Main_Menu').slideToggle();
  });
});