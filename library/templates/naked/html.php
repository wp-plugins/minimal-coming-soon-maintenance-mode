...
<?php
	...
	if (2 == $signals_csmm_options['disable_settings']) {
		if (1 == $signals_csmm_options['ignore_template']) {
			...
			echo 'body { background-color: #' . $signals_csmm_options['bg_color'] . ' }' . "\r\n";

			// Applying custom styles to the header text.
			echo 'h1 { font-family: "' . $signals_csmm_options['header_font'] . '", sans-serif; font-size: ' . $signals_csmm_options['header_font_size'] . 'px; color: #' . $signals_csmm_options['header_font_color'] . ' }' . "\r\n";

			// Applying custom styles to the secondary text.
			echo 'p { font-family: "' . $signals_csmm_options['secondary_font'] . '", sans-serif; font-size: ' . $signals_csmm_options['secondary_font_size'] . 'px; color: #' . $signals_csmm_options['secondary_font_color'] . ' }' . "\r\n";
